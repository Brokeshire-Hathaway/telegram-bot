...
export async function sendTokenAgent(
  ...
  setCache: <K extends keyof SendTokenCache, V extends SendTokenCache[K]>(
    key: K,
    value: V | Promise<V>,
  ) => Promise<V>,
  ...
  const sendTokenParams = await setCache(
    "sendTokenParams",
    getSendTokenParams(args.intent, accountUid, accountBalances),
  );
  ...
  if (
    sendTokenParams.recipientAddress === undefined ||
    !isAddress(sendTokenParams.recipientAddress)
  ) {
    console.log("recipientAddress is invalid");
    const message =
      "Please select a recipient from your contacts or enter a valid Ethereum address.";
  ...
  if (
    sendTokenParams.standardization === "erc20" &&
    (sendTokenParams.tokenAddress === undefined ||
      !isAddress(sendTokenParams.tokenAddress))
  ) {
  ...
  if (cache?.sendTokenPreview == null) {
    conversation.push({
      role: "assistant",
      content: `## sendTokenPreview Parameters\n\n${JSON.stringify(sendTokenParams)}`,
    });
    const userMessage = `Use "sendTokenPreview" to provide a preview of the transaction I am about to make using the format below.
...
**Total・**{total amount} {token symbol}"`;
    ...
    const sendTokenPreview =
      cache?.sendTokenPreview ??
      (await setCache("sendTokenPreview", getLatestMessageText(conversation)));
...
export async function getSendTokenParams(
  intent: string,
  accountUid: string,
  accountBalances: WalletTokenBalance[],
) {
  const userMessage = `Convert my intent into sendTokenPreview JSON parameters.
...
## sendTokenPreview JSON Parameters
{
  ...
  conversation = await sendTokenAssistant(conversation, {
  ...
  return JSON.parse(
    getLatestMessageText(conversation),
  ) as Partial<SendTokenPreviewParams>;
}
...
async function sendTokenAssistant(
  conversation: Conversation = [],
  config: SendTokenAgentConfig = {},
) {
