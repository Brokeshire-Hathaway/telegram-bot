...
export function startTelegramBot() {
  ...
  const groupBotTools = tools.filter(
    (tool) => tool.function.name === "getMarket",
  );
  ...
  groupBot.on("message:text", async (ctx) => {
    ...
    const replyMessageIsEmber =
      replyMessageUsername === process.env.TELEGRAM_BOT_USERNAME!;
    if (!replyMessageIsEmber) return;

    const messageText = ctx.message?.text;
    const replyToText = ctx.message?.reply_to_message?.text;
    const replyToMessage: ChatCompletionMessageParam[] | undefined = replyToText
      ? [{ role: "assistant", content: replyToText }]
      : undefined;
    await emberReply(ctx, messageText, {
      conversationHistory: replyToMessage,
      tools: groupBotTools,
    });
  });
  ...
  groupBot.on("my_chat_member", async (ctx) => {
    ...
    const newMemberIsEmber =
      newMemberUsername === process.env.TELEGRAM_BOT_USERNAME!;
    ...
    if (newMemberIsEmber && statusIsMember) {
      ...
      const chat: ChatFromGetChat & { username?: string } =
        await ctx.api.getChat(ctx.myChatMember.chat.id);
      const chatTitle = ctx.myChatMember.chat.title;
      const groupLink = chat.username
        ? `[${chatTitle}](https://t.me/${chat.username})`
        : `"${chatTitle}"`;
      const content = newGroupAddMessage(groupLink);
      const replyMessage = promoText ? `${content}\n\n ${promoText}` : content;
      sendFormattedMessage(
        ctx,
        Number(process.env.TELEGRAM_MAIN_CHAT_ID!),
        replyMessage,
      );
    }
  });
  ...
  privateBot.on("message:text", async (ctx) => {
    ...
    const onActivity = async (messageText: string) => {
      if (messageId == null) {
        const message = await sendFormattedMessage(
          ctx,
          ctx.chat.id,
          messageText,
          true,
...
async function emberReply(
  ctx: MyContext,
  userContent: string,
  config?: EmberReplyConfig,
) {
  let conversation: ConvoHistory = [
  ...
  if (toolCalls != null) {
    ...
    const toolMessages = (await runTools(toolCalls, availableFunctions)).filter(
      (toolMessage) => {
        ...
        console.log(toolMessage.content);
        console.log(
          toolMessage.content != null && JSON.parse(toolMessage.content),
        );

        if (
          toolMessage.content != null &&
          JSON.parse(toolMessage.content).value === "NO RESPONSE"
        ) {
  ...
  const content =
    getLatestMessageText(conversation) ?? "**Ember is sleeping 😴**";
  const replyMessage = promoText ? `${content}\n\n ${promoText}` : content;
